...
function TimeoutDialog($module: any): any {
  let options;
  let settings;
  ...
  function init() {
    ...
    options = {
      ...
      second: validate.string(lookupData('data-second')),
      seconds: validate.string(lookupData('data-seconds')),
      minute: validate.string(lookupData('data-minute')),
      minutes: validate.string(lookupData('data-minutes')),
  ...
  const startCountdown = ($countdownElement, $screenReaderCountdownElement) => {
    const getHumanText = (counter) => {
      ...
      let visibleMessage;
      if (counter < 60) {
        visibleMessage = `${counter} ${counter !== 1 ? settings.seconds : settings.second}`;
      } else {
        minutes = Math.ceil(counter / 60);
        visibleMessage = `${minutes} ${minutes === 1 ? settings.minute : settings.minutes}`;
      }
